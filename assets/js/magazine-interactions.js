(function() {
  ...
  // Do not measure on the first paint before webfonts/layout settle - a false
  // positive applies html.is-narrow-layout and hides Houston time + social links
  // until something else (e.g. now-playing:updated) remeasures.
  function bootNarrowLayoutSync() {
    scheduleNarrowLayoutSync();
    requestAnimationFrame(scheduleNarrowLayoutSync);
  }
  if (document.fonts && document.fonts.ready) {
    document.fonts.ready.then(bootNarrowLayoutSync).catch(bootNarrowLayoutSync);
  } else {
    bootNarrowLayoutSync();
  }
  window.addEventListener('load', scheduleNarrowLayoutSync, { once: true });
